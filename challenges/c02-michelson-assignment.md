...
df_q1 <- df_michelson %>%
  mutate(n = n_distinct(Distinctness))%>%
...
- Both plots share a similar trend overall.

Differences

- The first couple of days of the real data has a wider range as well as
  more outlet points than Michelson’s simulated data, while the latter
  has more outlet points than the real data over the last couple of
  days.
...
- The vacuum might create differences to the plots.
