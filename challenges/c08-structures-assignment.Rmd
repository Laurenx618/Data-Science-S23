...
-   To what extent can you tell what shape the distribution of the data has?
    -   The data has a vague normal distribution, which a higher count of strength in the middle and lower counts towards the left and the right sides.
...
-   Assuming your scopus is the probability of failure `POF` defined above, does your estimate exhibit real variability, induced variability, or both?
    -   The estimation exhibits a combination of real and induced variability. The real variability comes from the variation of strength in the generated training data, while the induced variability comes from the size of the Monte Carlo sampling method.
...
-   Does the confidence interval above account for uncertainty arising from *Monte Carlo approximation* of the POF? Why or why not?
    -   Yes, because the result doesn't vary according to the sample size and is not affected by the uncertainty of this method.
-   Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
