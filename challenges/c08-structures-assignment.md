...
- Assuming the scopus is the strength of an individual part made from
  ...
  - The observed variability is natural, because the individual parts
    are made from the same aluminum alloy.
...
- Can you confidently conclude that `POF < 0.03`? Why or why not.
  - No, because we need at least 100 data points to produce a POF at
    exactly 0.03.
...
- Does this confidence interval imply that `POF < 0.03`?
  - Yes, because the higher value is 0.0198, which is much smaller than
    ...
    the range that’s equal to or smaller than 0.0198.
- Compare this probability with your estimate from q2; is it more or
  less trustworthy?
  - This probability is more trustworthy, because the model uses a
    sample size of 10,000, which is much greater compared to the 25 data
    points in q2.
- Does the confidence interval above account for uncertainty arising
  from the *Monte Carlo approximation*? Why or why not?
  - No, because the confidence interval above is only specific to the
    one data set generated by the Monte Carlo sampling method. It cannot
    capture the variability caused by the sample size of the Monte Carlo
    approximation.
- Does the confidence interval above account for uncertainty arising
  from *limited physical tests* (`df_samples`)? Why or why not?
  - No, because the strength_meanlog and the strength_sdlog, both of
    which are used as sample generating parameters, are both derived
    from the original data set df_samples. Therefore, they are not
    capable to capture the uncertainty of the original sample.
- What could you do to tighten up the confidence interval?
  - Choose a greater sampling size for the Monte Carlo approximation.
- Can you *confidently* conclude that `POF < 0.03`? Why or why not?
  - No, because the data set that we used to generate the estimated POF
    was also generated by a training model instead of a collection of
    real data.
...
- How does this estimate compare with your Monte Carlo estimate above?
  - This estimate is slightly lower than the result generated using the
    Monte Carlo method (0.01898).
...
- Does this estimate have any uncertainty due to *limited physical
  ...
  - Yes, because more physical tests might be able to generate a
...
- Does the confidence interval above account for uncertainty arising
  ...
  - No, because the confidence interval only accounts for the model
    generated from the 25 tests in df_samples and cannot address the
    situations when the test size is greater or smaller than 25.
- Can you confidently conclude that `POF < 0.03`? Why or why not?
  - No. The sample size that is used to generate the fitted model is too
    small, which means we’re not sure if our model is actually a good
    fit to the question that we’re looking into.
